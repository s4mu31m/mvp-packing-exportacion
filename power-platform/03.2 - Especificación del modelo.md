...
Los códigos se generan automáticamente al crear el registro. El contador incremental es **independiente por entidad** y se resetea cada día.
 
| Entidad | Campo código interno | Formato | Ejemplo |
|---|---|---|---|
| Bin | `id_bin` | `YYYYMMDD-0001` | `20250315-0042` |
| Lote planta | `id_lote_planta` | `YYYYMMDD-L0001` | `20250315-L0007` |
| Pallet | `id_pallet` | `YYYYMMDD-P0001` | `20250315-P0003` |
 
> **Distinción entre identificador interno y código de negocio**
>
> `id_bin`, `id_lote_planta` y `id_pallet` son identificadores internos generados por el sistema (fecha + contador incremental). Su propósito es la trazabilidad técnica y la reportería detallada — no reemplazan al código operacional solicitado por el cliente ni al código que viene desde módulo campo.
>
> `bin_code` es el código de negocio del bin: puede provenir del módulo campo o formarse en recepción con información del operario. Es el código que el cliente y los operarios usan en el flujo operativo diario.
>
> Ambos campos son distintos y no intercambiables. La lógica de generación y validación de estos códigos se implementará prioritariamente en backend.
...
| Campo | Tipo | Notas |
|---|---|---|
| `id_lote_planta` | Texto | PK de negocio. Formato `YYYYMMDD-L0001` |
| `contador_incremental` | Entero | Resetea por día |
...
| `disponibilidad_camara_desverdizado` | Booleano | Nullable. Solo aplica si `requiere_desverdizado = true`. Registra si la cámara de desverdizado está disponible. Puede actualizarse cuando cambia la disponibilidad. |
...
>
> El agrupamiento por temporada se resuelve mediante filtros de rango de fechas sobre `fecha_cosecha` de los bins asociados. No existe campo `temporada` en el modelo.
