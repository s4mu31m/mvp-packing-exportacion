...
El MVP está **operativo y cerrado**. El flujo completo (Recepción → Desverdizado → Ingreso Packing → Proceso → Control → Paletizado → Cámaras) está implementado y funcional en ambos backends (SQLite y Dataverse). No quedan gaps bloqueantes pendientes.
...
|--------|:--------------:|
| ~~`filtro_productor` en consulta jefatura (Dataverse)~~ | ~~Sí~~ → Resuelto 2026-04-04 |
...
Los PR #43 y #44 cerraron el flujo Dataverse completo e integraron `crf21_etapa_actual` como fuente de verdad del avance operativo del lote. El 2026-04-04 se resolvió la última deuda bloqueante: `crf21_codigo_productor` fue agregado a `crf21_lote_plantas` en Power Apps y se puebla automáticamente al cerrar el lote. El MVP no tiene gaps bloqueantes pendientes.
